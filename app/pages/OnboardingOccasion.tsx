...
function OnboardingOccasionInner() {
  ...
  const handleNext = () => {
    if (selected) {
      const params = new URLSearchParams({ occasion: selected });
      if (template) params.set('template', template);
      router.push(`/onboarding/szablon?${params.toString()}`);
    }
  };
      ...
      <div className="max-w-5xl w-full">
        ...
        <div className="flex items-center justify-center gap-2 mb-12">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#7C3AED] text-white flex items-center justify-center text-sm font-semibold">1</div>
            <span className="text-sm font-medium text-[#111827]">Okazja</span>
          </div>
          <div className="w-12 h-0.5 bg-[#D1D5DB]" />
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#E5E7EB] text-[#9CA3AF] flex items-center justify-center text-sm font-semibold">2</div>
            <span className="text-sm text-[#9CA3AF]">Szablon</span>
          </div>
          <div className="w-12 h-0.5 bg-[#D1D5DB]" />
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#E5E7EB] text-[#9CA3AF] flex items-center justify-center text-sm font-semibold">3</div>
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-[#6B7280] text-lg">Wybierz typ wydarzenia, aby dostosować szablon</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-12">
          {occasions.map((occasion) => (
            <Card key={occasion.id} onClick={() => setSelected(occasion.id)}
              className={`relative p-6 text-center cursor-pointer transition-all bg-gradient-to-br ${occasion.gradient} border-4 hover:scale-105
                ${selected === occasion.id ? 'border-white shadow-2xl scale-105' : 'border-transparent'}`}>
        ...
        <div className="flex justify-center gap-4">
          <Button variant="outline" size="lg" onClick={() => router.push('/')}>Wróć</Button>
          <Button size="lg" onClick={handleNext} disabled={!selected}
            className="rounded-full bg-[#7C3AED] hover:bg-[#5B21B6] px-8">Dalej</Button>
        </div>
      </div>
    </div>
  );
